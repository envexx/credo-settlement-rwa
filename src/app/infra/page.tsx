...
const sections: DocsSectionGroup[] = [
  {
    group: "Get started",
    links: [
      ["Overview", "overview"],
          ...
          >
            <div className="mb-7 flex items-center gap-3">
              ...
              <Breadcrumb>
                <BreadcrumbList className="font-mono text-[11px]">
                  ...
                  <BreadcrumbSeparator />
                  <BreadcrumbItem>Protocol</BreadcrumbItem>
                  <BreadcrumbSeparator />
                  <BreadcrumbItem>
            ...
            <DocSection>
              ...
              <p className="mt-5 text-lg leading-8 text-muted-foreground">
                Credo is a proof-triggered delivery-versus-payment settlement
                layer. Official USDC remains final on Ethereum, the ERC-1155
                remains on Creditcoin, and only cryptographic evidence crosses
                chains.
              </p>
              ...
              <blockquote className="my-9 rounded-r-md border-l-2 border-primary bg-primary/5 p-5 text-sm leading-6">
                <strong className="text-primary">Trust boundary:</strong> The
            ...
            <p className="mt-4 text-sm leading-7 text-muted-foreground">
              ...
              escrows the RWA on Creditcoin; the bound buyer pays official USDC
              on Sepolia. The recommended API-assisted path validates parameters
              and indexes the sale, while both users still sign their own wallet
              transactions.
            </p>
...
await fetch("/api/v1/sales/index", {
  ...
  body: JSON.stringify({ saleId, creationTxHash })
            ...
            <p className="mt-4 text-sm leading-7 text-muted-foreground">
              Use the API-assisted path below for validation, authentication,
              and worker tracking. A direct-contract integration may call the
              same ABI, but must calculate every raw value and source block
              itself and still index the resulting sale before using
              Credo&apos;s payment API.
            </p>
            <ol className="mt-5 space-y-3">
              {[
            ...
            <p className="mt-4 text-sm leading-7 text-muted-foreground">
              Registering a payment returns <code>202 Accepted</code>. This
              means proof processing has started, not that settlement has
              completed. Poll <code>GET /api/v1/sales/:saleId/settlement</code>.
              The sale status is <code>OPEN</code>, <code>SETTLED</code>, or{" "}
...
const state = await fetch(
  \`/api/v1/sales/\${saleId}/settlement\`
            ...
            <p className="mt-4 text-sm leading-7 text-muted-foreground">
              The prepare endpoint builds a default payment window of 7,200
              blocks from the next Sepolia block. JSON integer values remain
              decimal strings; convert them to <code>BigInt</code> only when
              encoding the Creditcoin transaction. Attestcoin chain key{" "}
              <code>1</code> identifies the proof source, while EVM chain ID{" "}
              <code>11155111</code> identifies Sepolia. They are different
            ...
            <p className="mt-4 text-sm leading-7 text-muted-foreground">
              ...
              functions and events below; the complete compiler-generated ABIs
              are available for tooling and contract verification.
            </p>
...
export const settleRwaAbi = parseAbi([
  ...
  "event AssetReclaimed(bytes32 indexed saleId,address indexed seller,address indexed asset,uint256 tokenId,uint256 amount)"
            ...
            <p className="mt-4 text-sm leading-7 text-muted-foreground">
              Settlement binds the source chain key, block window, official
              token emitter, designated Buyer, recipient, exact raw amount,
              successful receipt status, and a globally unused query ID.
            </p>
            <p className="mt-4 text-sm leading-7 text-muted-foreground">
              ...
              authority: anyone may submit evidence, but only a proof accepted
              by the native verifier can release escrow. The operator can
              allowlist assets and pause new sales, but cannot force an existing
              sale to settle or redirect its asset.
            </p>
            <blockquote className="mt-5 rounded-r-md border-l-2 border-primary bg-primary/5 p-5 text-sm leading-6">
              ...
              Sepolia payment is irreversible before Creditcoin release
              completes. The reclaim deadline covers the source payment window
              plus a 24-hour grace so the seller cannot reclaim during the
              expected proof period.
            </blockquote>
...
function DocSection({ children }: { children: React.ReactNode }) {
  return (
    <section className="rounded-xl border bg-card/55 px-5 py-6 shadow-sm sm:px-7 sm:py-8">
      {children}
    </section>
  );
}
...
function DocHeading({
  ...
  return (
    <h2
      id={id}
      className="mt-12 scroll-mt-28 rounded-xl border bg-card/55 px-5 py-5 text-2xl font-semibold tracking-[-.035em] shadow-sm sm:px-7"
    >
      {children}
    </h2>
  );
